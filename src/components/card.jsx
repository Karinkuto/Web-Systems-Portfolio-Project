export default function Card({ children, image, reverse = false }) {
  ...
  return (
    <div className={`group flex flex-col hover:cursor-pointer ${flexDirection} rounded-[24px] md:p-[24px] p-[16px] ring-border ring-[1px] bg-white gap-[24px] md:max-w-[1156px] sm:max-w-[554px] md:h-fit sm:h-fit hover:ring-cardBorderHover hover:shadow-card hover:ring-[2px] transition-all ease-in-out`}>
      {image && (
      ...
      <div className="flex flex-col md:gap-[24px] gap-[12px] h-full w-full flex-1 basis-1/2 md:p-[24px]">{children}</div>
    </div>
  );
}
...
Card.Subtitle = function CardSubtitle({ place, dev, year }) {
  return (
    <div className="w-fit h-fit flex items-center gap-[12px] tracking-[0.03em] py-[8px] md:text-[18px/24px] text-[13px]/[16px]">
      {place && <span className="text-cardSubtitleMain uppercase md:font-medium font-semibold">{place}</span>}
      <span className="w-[8px] h-[8px] rounded-full bg-cardSubtitleDot"/>
      {dev && <span className="text-cardSubtitleRest md:font-normal font-semibold">{dev}</span>}
      <span className="w-[8px] h-[8px] rounded-full bg-cardSubtitleDot"/>
      {year && <span className="text-cardSubtitleRest md:font-normal font-semibold">{year}</span>}
    </div>
  );
};
